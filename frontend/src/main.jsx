...
function App(){ 
 ...
 const signup = e => { 
  ...
  .then(x => { 
    alert(x.message); 
    setForm({}); 
    setPage("login");  // ২. রিডাইরেক্ট করে লগইন পেজে নিয়ে যান
  }); 
};
 ...
 return <main><header><b>SkillSwap</b><button onClick={()=>setPage("new")}>+ Create Swap</button> 
...
/>{posts.filter(p => 
  p.learn_skill.toLowerCase().includes(search.toLowerCase()) 
   ...
   <button onClick={()=>alert(`Contact ${p.name}: ${p.phone}`)}>Connect / Call</button> 
  </article>)}</section>} 
 </main> 
} 
